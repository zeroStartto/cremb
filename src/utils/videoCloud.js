...
export default {
  ...
  videoUpload(config) {
    if (config.type === 'COS') {
      ...
      return this.uploadMp4ToLocal(config.evfile, config.res, config.uploading)
    } else if (config.type === 'OBS') {
      return this.obsHttp(config.evfile, config.res, config.uploading)
    } else if (config.type === 'US3') {
      return this.us3Http(config.evfile, config.res, config.uploading)
    } else {
